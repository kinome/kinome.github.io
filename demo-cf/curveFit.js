/*global $, window, google, jQuery, requireJS, M, KINOMICS*/
...
(function () {
    ...
    createFigure = function () {
        ...
        var eq, chart, data, dataTable, i, j, length, max, min, options, params, tempElem;
        ...
        $('<dd/>').append("<i>W.W. runs p-val: " + (Math.round(data.WWtest[0] * 100) / 100) + "<i>").appendTo(tempElem);
    ...
    parseAndFit = function () {
        ...
        dataArr.map(function (p) {
            p = p.split('\t');
            currentData.x_values.push(parseInt([p[0]], 10));
